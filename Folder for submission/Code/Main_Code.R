...
For each year we see that the trend is similar to the others, so it seems reasonable to try to extract a seasonal pattern from the data.
...
We know that autocorrelation occurs when the effect of a variable is spread over time, in these cases, most of the autocorrelations fall within the confidence bands, indicating that the data does not show significant correlation for most lags. 
...
In this section, we perform a train-test split to prepare the data for model training and evaluation. We divide the time series data for both Baccala Mantecato and Baccala Vicentina into training and testing sets, with 80% of the data allocated for training and the remaining 20% for testing.
```{r train_test_split}
prop <- 0.8
...
- Regarding the Month variables, they capture seasonal effects by representing deviations from the baseline month (January). Some months show statistically significant effects (e.g., February, August, September, October, and December. For example, December shows a substantial positive effect, indicating a peak in that month, while months like February and October have negative coefficients, reflecting significant declines compared to January.
                                                                                                                                                                                 ...
                                                                                                                                                                                 The DW tests suggests autocorrelation in the residuals, but we also tried to fit a linear regression with ARIMA errors, but it led us to overfitting.
                                                                                                                                                                                 ...
                                                                                                                                                                                 Furthermore, the fish_cons variable shows a large p-value, suggesting that it can be removed from the model without significantly affecting the results.
